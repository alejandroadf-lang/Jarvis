...
actually request a tranche it shouldn't have, did the Critic actually flag
a tiny idea — rather than eyeballing the reply text.
...
- **CFO tranche judgment** — requests a tranche when a milestone is done,
  refuses when it isn't.
...
`ventures.json`'s `pendingTranche` actually get set, did the ledger
balance actually change) rather than keyword-matching the reply, since
