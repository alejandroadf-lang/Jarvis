...
export async function synthesizeSpeech(text, { language = 'en', format = 'opus', provider = null } = {}) {
  ...
  recordSpend(result.costUsd);
  // Every voice note the advisor sends says inside the file that a machine
  // made it. See markSynthetic for what and why.
  const buffer = markSynthetic(result.buffer, { provider: voice.id, model: safeModel(voice), language });
  return { ...result, buffer, marked: buffer !== result.buffer, provider: voice.id, ms: Date.now() - startedAt };
}

function safeModel(voice) {
  try {
    return typeof voice.model === 'function' ? voice.model() : null;
  } catch {
    return null;
  }
}

/** Whether synthesised audio is marked as such in its container. On by default. */
export function audioMarkingEnabled() {
  return (process.env.TRAVEL_VOICE_MARK_AUDIO || '').trim().toLowerCase() !== 'false';
}

/** The comments written into a synthesised voice note. */
export function syntheticAudioComments({ provider = null, model = null, language = null } = {}) {
  return [
    'AI_GENERATED=true',
    'SYNTHETIC_VOICE=true',
    // The wording the transparency rule asks for, in the file itself.
    'DISCLOSURE=This audio was generated by an AI system (EU AI Act, Article 50)',
    `GENERATOR=${[provider, model].filter(Boolean).join('/') || 'travel-voice-advisor'}`,
    ...(language ? [`LANGUAGE=${language}`] : []),
    'ORIGIN=Jarvis Travel Voice Advisor',
  ];
}

/**
 * Writes the AI-generated marker into an Ogg Opus stream's comment header.
 *
 * A container comment is machine-readable and survives the file being
 * saved and forwarded; it does not survive transcoding, and whether
 * WhatsApp re-encodes a voice note on its way through is something to test
 * on the real channel rather than assume. A marker inside the signal
 * itself would survive that too, and needs a watermarking model this
 * server does not have — a vendor with one is the next step, not a
 * substitute for the spoken disclosure on first contact (see consent.js).
 * Audio that is not Ogg — the MP3 fallback — passes through unmarked.
 */
export function markSynthetic(buffer, meta = {}) {
  if (!audioMarkingEnabled()) return buffer;
  try {
    return tagOggOpus(buffer, syntheticAudioComments(meta));
  } catch (err) {
    console.warn(`Travel voice: could not mark a voice note as synthetic (${err.message}); sending it unmarked.`);
    return buffer;
  }
}
